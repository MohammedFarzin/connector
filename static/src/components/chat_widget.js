odoo.define("connector.components.chat_widget", function (require) {
    "use strict";
    ...
    const { Component, useState, useRef, onMounted, onWillDestroy, markup } = require("@odoo/owl");
    const { useService } = require("@web/core/utils/hooks");
    const { ChatMessage } = require("connector.components.chat_message");
    const { ChatInput } = require("connector.components.chat_input");
    ...
    class FloatingWidget extends Component {
        ...
        setup() {
            ...
            // useService("rpc") works on BOTH v16 and v18 (unlike the v18-only
            // direct import from @web/core/network/rpc).
        ...
        async onSendMessage(text) {
            ...
            try {
                const result = await this._rpc("/crm_assistant_connector/message", {
                    message: text,
        ...
        async onResetSession() {
            try {
                const result = await this._rpc("/crm_assistant_connector/reset", {
                    session_id: this.sessionId,
                });
                if (result.session_id) {
                    this.sessionId = result.session_id;
        ...
        async _restoreSession(sessionId) {
            try {
                const result = await this._rpc("/crm_assistant_connector/restore", {
        ...
        async _loadSessions() {
            ...
            try {
                const result = await this._rpc("/crm_assistant_connector/sessions", {});
